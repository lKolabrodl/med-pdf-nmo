...
- На втором этапе общий `scorers/legacy.ts` удален: scorer-ы разнесены по
  тематическим модулям, а их прежний порядок сохранен в `answer-score.ts`.
...
- Следующий безопасный рефактор — поэтапно включать TypeScript `strict` и
  делить только самые крупные тематические модули с zero-delta проверкой.
