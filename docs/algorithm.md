...
6. Score each answer with non-LLM evidence signals:
   ...
   Definition-exact evidence handles `X это ...`, `Под X понимают ...`, and dash-definition rows. The answer phrase must be in the same definition fragment and the fragment label must match the question term, allowing a one-edit OCR typo in the label.
