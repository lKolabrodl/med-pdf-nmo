...
1. Extract PDF text page-by-page with `pdfjs-dist`, stripping generic running boilerplate (`страница N из M` footers, standalone page-number lines, and URL/source lines) so it does not pollute chunks or numeric matching. The running guideline header is intentionally kept because it carries the disease name. The "Список литературы" (bibliography) section is also removed — it is ~22% of the text, is pure citation noise that can never be an answer, and is cleanly bounded from its heading to the next "Приложение ..." section so all clinical appendices are kept; removing it sharpens term IDF and cuts false Latin/gene matches from author/journal names. More pure-metadata blocks are removed for the same reason: the table of contents (detected by its dotted/ellipsis leaders, since most TOCs have no "Содержание"/"Оглавление" heading), the front-matter appendix list, and appendix А1 "Состав рабочей группы" (working-group names). Methodology (А2), related-documents (А3), and the quality-criteria checklist were tested and NOT removed — they regressed held-out accuracy because their text is close to clinical vocabulary; clinical appendices Б/В/Г are always kept, and "рабочая группа" glossary definitions are kept as content.
   The extractor also detects real `Список сокращений` / abbreviation-list blocks when the heading is followed by multiple abbreviation entries. It strips parenthetical tails inside those entries, including service fragments such as `(Код АТХ: A02BC01)` and generic metadata-like notes such as `(хв 1223)`, then exposes document-specific `{ abbr, expansion, page }` aliases in `pdfText.abbreviations`. The abbreviation block is bounded by common next-section headings (`Термины и определения`, numbered sections, bibliography, appendices), using whitespace/end-of-line boundaries instead of JavaScript `\b` for Cyrillic text. These aliases are metadata, not answer-key data.
...
6. Score each answer with non-LLM evidence signals:
   ...
   - `abbreviation_alias_window` for multi-answer questions where an answer matches a cleaned document-specific abbreviation expansion, and the abbreviation itself occurs outside the glossary page in a local window with question-context support. The scorer ignores single-answer questions, two-character abbreviations, glossary-page hits, reverse generic matches such as `с-м = синдром` for `синдром X`, and strong literal hits where the answer is already directly found.
   ...
   Additional conservative structural passes run only when their complete contracts are satisfied:
   ...
   - These passes use fresh bounded fragments rather than global BM25 windows. Atomic recommendation segments stop at grade/comment lines, new bullets, numbered headings, and unconnected line continuations.
...
10. Select:
   ...
   - `multi`: answers passing absolute, relative, or score-gap thresholds, followed by a hard lower bound of two distinct answers whenever two variants exist.
...
- `src/predictor/pipelines/score-adjustment-pipeline.ts`: generic sequential runner for set-level and post-scoring processors.
- `src/predictor/pipelines/score-adjustment-processors.ts`: nine individually named processors, assembled in an explicit behavior-significant order by the composition root.
...
- `scripts/dataset-manifest.ts`: frozen deduplicated PDF-group composition, split membership, and aggregate PDF fingerprint.
- `scripts/validate-dataset.ts`: corpus-integrity, duplicate-group, split-leakage, and label/cardinality checks; this script is development-only and is never imported by runtime.
...
- minimum selected answers in `multi` mode: `2`. This is an intentional task prior, not inferred answer-key information: every validated keyed multi case has at least two correct answers, and a real multi-choice interaction requires more than one choice. Additional cardinality is still derived from PDF evidence.
