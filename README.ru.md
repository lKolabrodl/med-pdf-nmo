...
Цифры получены на очищенном от дублей локальном корпусе: 45 PDF-групп и 2 684 keyed cases. Это не гарантия качества на новом PDF, а текущий ориентир после полного финального прогона.

| Набор | Exact accuracy | Single-answer | Multi-answer exact set |
| --- | ---: | ---: | ---: |
| Все keyed cases | `74.78%` (`2007/2684`) | `82.61%` (`1506/1823`) | `58.19%` (`501/861`) |
| Train split | `69.37%` (`1069/1541`) | `78.92%` | `51.67%` |
| Dev split | `79.35%` (`415/523`) | `83.92%` | `68.59%` |
| Замороженный holdout regression | `85.00%` (`459/540`) | `89.90%` | `72.73%` |
| Внешний transfer baseline | `80.00%` (`64/80`) | `88.41%` | `27.27%` |
...
Predictor был заморожен до добавления двух текущих внешних PDF. Первый зафиксированный результат — `64/80`: `43/50` и `21/30` по отдельным PDF. После чтения этих ответов runtime-правила не менялись. Исторический holdout остаётся regression suite, поскольку его ответы использовались в предыдущей разработке.
