...
Цифры получены на очищенном от дублей локальном корпусе: 44 PDF-группы и 2 674 keyed cases. Это не гарантия качества на новом PDF, а текущий ориентир после полного финального прогона.

| Набор | Exact accuracy | Single-answer | Multi-answer exact set |
| --- | ---: | ---: | ---: |
| Все keyed cases | `74.27%` (`1986/2674`) | `82.26%` (`1479/1798`) | `57.88%` (`507/876`) |
...
| Внешний transfer regression | `61.43%` (`43/70`) | `77.27%` | `34.62%` |
...
Последний transfer-раунд сохранил dev `415/523`, улучшил замороженный holdout с `458` до `459/540` и внешний PDF с `27` до `43/70`. Ответы holdout и внешнего набора уже использовались в анализе, поэтому это regression/stress suites, а не слепая оценка обобщения.
