...
function subjectBoundNumericClauses(text) {
  return String(text ?? "")
    .split(/\s*;\s*|(?<=[.!?])\s+/u)
...
function numericOptionValues(answers) {
...
function numericComparatorNearValue(text, value) {
 ...
 */
export function bestSubjectBoundNumericClauseSupport({
  mode,
  pages,
  topQuestionPages,
  question,
