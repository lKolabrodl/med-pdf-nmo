...
const COORDINATE_RELATIONAL_GENERIC_TOKENS = new Set(
  [
 ...
 */
export function hasCoordinateRelationalRowCue(question) {
  const raw = String(question ?? "").toLowerCase();
  const cueGroups = [
    ["тип", "поврежден"],
    ["механизм"],
...
export function hasCoordinateComparisonTableCue(question) {
  const raw = String(question ?? "").toLowerCase();
  return (
...
function clusterCoordinateXs(values, tolerance = 18) {
  const clusters = [];
...
function coordinateRelationalAnchors(lines) {
  const candidates = [];
  const xValues = [];
  for (let index = 0; index < lines.length; index += 1) {
...
function nearestCoordinateAnchorIndex(anchors, x) {
  let bestIndex = -1;
  let bestDistance = Infinity;
...
function coordinateRelationalColumnIndex(anchors, x) {
...
function coordinateRelationalHeader(cells) {
  const raw = cells
    .map((cell) => cell.text)
    .join(" ")
    .toLowerCase();
...
function coordinateRelationalRows(page) {
...
function coordinateCodeKey(text) {
  return String(text ?? "")
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, "");
}

function coordinateEditSimilarity(left, right) {
...
function coordinateCodeSimilarity(left, right) {
  const a = coordinateCodeKey(left);
  const b = coordinateCodeKey(right);
...
function coordinateInlineAliases(pages) {
  const aliases = [];
  for (const page of pages) {
...
function coordinateCellAliasText(cellText, pageNumber, aliases) {
...
function coordinateRelationalAliasFootnote(row) {
...
function coordinateRelationalTableRegion(rows, localHeader) {
  const ordered = [...rows]
...
function coordinateRelationalExplicitHeaderRow(row) {
 ...
 */
export function buildCoordinateRelationalRowsByPage(pages, topQuestionPages, scanAllExplicitTables = false) {
  const byPage = new Map();
...
function coordinateRelationalFocusTokens(question, focusTokens, answerTokens) {
  return coordinateTableFocusTokens(question, focusTokens, answerTokens).filter(
    (token) => !COORDINATE_RELATIONAL_GENERIC_TOKENS.has(token),
  );
}

function coordinateComparisonTargetTokens(question) {
...
function coordinateComparisonPolarityCompatible(question, valueText) {
  const normalizedQuestion = normalizeForSearch(question);
